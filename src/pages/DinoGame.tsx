...
const DinoGame: React.FC = () => {
  ...
  const handleSuccessfulObstaclePass = useCallback(async (obstacleIndex: number) => {
    ...
    try {
      ...
      const response = await fetch(`https://api.horizon777.online/api/dino/${gameState._id}/jump`, {
        method: 'POST',
        headers: {
  ...
  const endGame = useCallback(async () => {
    if (gameState && !gameOver) {
      ...
      try {
        ...
        const response = await fetch(`https://api.horizon777.online/api/dino/${gameState._id}/end`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json'
  ...
  const startGame = useCallback(async () => {
    try {
      ...
      const response = await fetch('https://api.horizon777.online/api/dino/start', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
